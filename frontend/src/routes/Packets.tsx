            ...
            {selectedPacket ? (
              <div className="p-4">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <p className="text-sm text-muted-foreground">{formatDateTime(selectedPacket.received_at)}</p>
                </div>
                ...
                <pre className="text-xs bg-neutral-800 p-2 my-2 rounded-md whitespace-pre-wrap break-all">
                  {JSON.stringify(JSON.parse(selectedPacket.packet_data), null, 2)}
                </pre>
                  ...
                  {Object.entries(JSON.parse(selectedPacket.packet_data)).map(
                    ([key, value]) => (
                      ...
                      >
                        ...
                        <p className="font-mono break-all text-right flex-1">{String(value)}</p>
                      </div>
                    )
                  )}
                </div>
